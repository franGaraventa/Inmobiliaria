...
public class GeneradorPDF {
	...
	private static final Font bold = new Font(FontFamily.HELVETICA, 12, Font.BOLD);
	...
	public void generarPDFFactura() {
		...
		if (opcion == JFileChooser.APPROVE_OPTION) {
			try {
				...
				Chunk cTitulo = new Chunk("RECIBO DE PAGO",bold);
				titulo.add(cTitulo);
				...
				Chunk cpersona_2 = new Chunk(contrato.getLocatario().getNombre().toUpperCase()+","+contrato.getLocatario().getApellido().toUpperCase());
				cpersona_2.setUnderline(0.1f, -2f);
				persona.add(new Chunk("Recibi de: "));
				persona.add(cpersona_2);
				...
				Chunk cpesos_1 = new Chunk(pesos_es.toUpperCase());
				cpesos_1.setUnderline(0.1f, -2f);
				pesos_escrito.add(new Chunk("La cantidad de PESOS: "));
				pesos_escrito.add(cpesos_1);
				documento.add(pesos_escrito);
				/*-------------------------------------------------------------------------------------------------------------*/
				//UBICACION y FECHA
				Paragraph ubicacion = new Paragraph();
				ubicacion.add(new Chunk("en conceptos del alquiler que se encuentra ubicado en "));
				Chunk cubicacion = new Chunk(contrato.getLocacion().getUbicacion().getDireccion().toUpperCase());
				cubicacion.setUnderline(0.1f, -2f);
				ubicacion.add(cubicacion);
				ubicacion.add(new Chunk(" de la ciudad de "));
				Chunk ciudad = new Chunk(contrato.getLocacion().getUbicacion().getCiudad().toUpperCase());
				ubicacion.add(ciudad.setUnderline(0.1f, -2f));
				ubicacion.add(new Chunk(". Correspondiente al mes "));
				Chunk mes = new Chunk(Fechas.getMes(calendar.get(Calendar.MONTH)).toUpperCase());
				ubicacion.add(mes.setUnderline(0.1f, -2f));
				ubicacion.add(new Chunk(" que vence el "));
				Chunk fecha = new Chunk(contrato.getFechaMaxPago() +"/"+(calendar.get(Calendar.MONTH)+1)+"/"+calendar.get(Calendar.YEAR));
				ubicacion.add(fecha.setUnderline(0.1f, -2f));
				documento.add(ubicacion);
				/*-------------------------------------------------------------------------------------------------------------*/
				//ALQUILER
				Paragraph alquiler = new Paragraph();
				Chunk calquiler = new Chunk(String.format("%.2f", contrato.getPrecio().getPrecio()));
				alquiler.add("Alquiler $: ");
				alquiler.add(calquiler.setUnderline(0.1f, -2f));
				documento.add(alquiler);
				/*-------------------------------------------------------------------------------------------------------------*/
				//EXPENSAS
				Paragraph expensas = new Paragraph();
				Chunk cexpensas = new Chunk(String.valueOf(contrato.getPrecio().getExpensas()));
				expensas.add("Expensas $: ");
				expensas.add(cexpensas.setUnderline(0.1f, -2f));
				documento.add(expensas);
				/*-------------------------------------------------------------------------------------------------------------*/
				//PRECIO FINAL
				Paragraph precio_final = new Paragraph();
				Chunk monto = new Chunk(String.valueOf(pago.getMonto()));
				Chunk recargo = new Chunk(String.valueOf(pago.getRecargo()));
				precio_final.add("Total $: ");
				precio_final.add(monto.setUnderline(0.1f, -2f));
				precio_final.add("         Recargo $: ");
				precio_final.add(recargo.setUnderline(0.1f, -2f));
				...
				JOptionPane.showMessageDialog(null, "Hubo un error al generar la factura");
			}
		}
	}
	...
	public void generarPDFContrato() {
		...
		if (opcion == JFileChooser.APPROVE_OPTION) {
			try {
				...
				Paragraph titulo = new Paragraph("CONTRATO DE LOCACION",bold);
				titulo.setAlignment(Element.ALIGN_CENTER);
				documento.add(titulo);
				...
				clausula_1.add(new Chunk("PRIMERA (OBJETO/DESTINO):",bold));
				clausula_1.add(" EL/LA 'LOCADOR/A' cede en locacion al 'LOCATARIO/A, que acepta ocupar en tal caracter, el inmueble ubicado en la calle "+
				...
				clausula_2.add(new Chunk("SEGUNDA (PLAZO):",bold));
				clausula_2.add(" Las partes acuerdan que el plazo de vigencia de la locacion sera de "+meses_contrato+" ( "+meses.toUpperCase()+") meses a contar desde el dia "+
				...
				clausula_3.add(new Chunk("TERCERA (PRECIO Y AJUSTE):",bold));
				clausula_3.add(" Las partes convienen un canon locativo de PESOS ($ "+valorpago+") mensuales para el primer año de contrato "
				...
				CuentaBancaria cb = contrato.getLocador().getCuentaBancaria();
				clausula_4.add(new Chunk("CUARTA (FECHA/LUGAR DE PAGO):",bold));
				clausula_4.add(" EL/LA LOCATARIO/A se obliga a abonar el alquiler convenido por mes entero y adelantado, entre el 1 y el "+contrato.getFechaMaxPago()+
						" de cada mes. El pago se efectuara por transferencia electronica o deposito bancario en la Cuenta Nº "+cb.getNrocuenta()+" del Banco "+cb.getBanco()+", CUIT"+cb.getTipoCuit()+"-"+cb.getDocempCuit()+"-"+cb.getValidadorCuit()
						+" CBU "+cb.getCbu()+", titular del LOCADOR/A \n"
				...
				clausula_5.add(new Chunk("QUINTA (INTERESES):",bold));
				clausula_5.add(" La mora en el pago de los alquileres se producira de forma automatica por el mero transcurso del tiempo y sin necesidad de interpelacion ni gestion previa"
				...
				clausula_6.add(new Chunk("SEXTA (PROHIBICIONES/INTRANSFERIBILIDAD):",bold));
				clausula_6.add(" El presente contrato de locacion es instransferible. Le queda prohibido al LOCATARIO/A cederlo o subarrendarlo total o parcialmente, sin"
				...
				clausula_7.add(new Chunk("SEPTIMA (LA CLAUSULA DE GARANTIA)",bold));
				documento.add(clausula_7);
				documento.add( Chunk.NEWLINE );
				//CLAUSULA 8
				Paragraph clausula_8 = new Paragraph();
				clausula_8.add(new Chunk("OCTABA (IMPUESTOS/SERVICIOS/EXPENSAS):",bold));
				clausula_8.add(" Son obligaciones del LOCATARIO/A a) El pago de los siguientes servicios: I) electricidad, gas, agua potable, II) cargas y contribuciones asociadas al "
				...
				clausula_9.add(new Chunk("NOVENA (DEPOSITO):",bold));
				clausula_9.add(" En garantia de las obligaciones contraidas mediante este contrato, el/la LOCATARIO/A da en deposito al LOCADOR/A la suma equivalente a un mes de alquiler correspondiente al valor del "
				...
				clausula_11.add(new Chunk("DECIMAPRIMERA (INCUMPLIMIENTO):",bold));
				clausula_11.add(" La violacion por parte de EL/LA LOCATARIO/A de cualquiera de las obligaciones que asume en el presente, dara derecho a EL/LA LOCADOR/A para optar entre exigir su cabal "
				...
				clausula_12.add(new Chunk("DECIMASEGUNDA (FALTA DE PAGO):",bold));
				clausula_12.add(" La falta de pago de 2 meses de alquiler consecutivos,dara derecho al LOCADOR/A a, previos los tramites establecidos por la ley, considerar irrevocablemten rescindido el presente "
				...
				clausula_13.add(new Chunk("DECIMATERCERA (PRIMER MES):",bold));
				clausula_13.add(" EL/LA LOCATARIO/A abona en este acto la cantidad de PESOS "+valor1erMes.toUpperCase()+" ($"+valorpago+"-) en concepto del alquiler correspondiente al mes de "+mespago+" de "+aniopago+".\n"
				...
				clausula_14.add(new Chunk("DECIMACUARTA (ENTREGA DE LLAVES):",bold));
				clausula_14.add(" Al vencimiento del contrato, el/la LOCATARIO/A debera devolver las llaves al LOCADOR/A. La recepcion por parte de/la LOCADOR/A del inmueble arrendado, aun sin reserva "
				...
				clausula_15.add(new Chunk("DECIMAQUINTA (RESCISION ANTICIPADA):",bold));
				clausula_15.add(" De acuerdo a lo establecido en el art. 1220 del Codigo Civil y Comercial, el/la LOCATARIO/A puede resolver el contrato en cualquier momento, sin costo alguno de su parte, "
				...
				clausula_16.add(new Chunk("DECIMA SEXTA (RENOVACION):",bold));
				clausula_16.add(" Dentro de los ultimos (3) meses del contrato cualquiera de las partes puede convocar, mediante notificacion fehaciente, a la otra a conversar sobre la renovacion de la locacion. La parte notificada "
				...
				clausula_17.add(new Chunk("DECIMA SEPTIMA (DOMICILIOS DE LAS PARTES):",bold));
				clausula_17.add("\nLas partes establecen los siguiente domicilios para todo lo relativo al presente contrato:\n a) LOCADOR/A - en "+contrato.getLocador().getDireccion()+" y "+contrato.getLocador().getEmail()+"\n"+
				...
				clausula_18.add(new Chunk("DECIMA OCTAVA:",bold));
				clausula_18.add(" Las partes se someten a la jurisdiccion y competencia de la Justicia Ordinaria en lo Civil de la Capital Federal con expresa renuncia a cualquier otro fuero y/o jurisdiccion que pudiera corresponderles. "
